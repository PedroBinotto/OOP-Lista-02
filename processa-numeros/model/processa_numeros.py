...
def primeiro_e_ultimo(numeros):
	...
	"""
	if len(numeros) >= 2:
		result = [numeros[0], numeros[len(numeros) - 1]]
		return result 
...
def media(numeros):
	...
	if len(numeros) == 0:
		...
		result = sum(numeros) / len(numeros)
	return result
...
def multiplica_por_fator(numeros, fator):
	...
	"""
	numeros = [(x * fator) for x in numeros]	# Não ta funcionando
...
def multiplicado_por_fator(numeros, fator):
	...
	return [(x * fator) for x in numeros.copy()]

# DAQUI PRA BAIXO EU AINDA NAO FIZ BIXO
